...
Though it is [worryingly straightforward](https://arkandas.com/blog/mifare_classic_cracking) to exploit the weak cryptography of the Mifare Classic standard to obtain the private keys ithout permission, I got the message that I was not meant to be reading camcards without permission.
...
This came with an unexpected advantage - by only storing the most basic possible information, the system now works with _any_ RFID card in existence, allowing visitors to use the system as well as group members. The downside: there is no way to tell _a priori_ which crsid has what UID, meaning that users will need to complete an annoying registration step to start using the system. 
## Encoder

I found out the hard way that cheap encoders wear out _very_ quickly - only six months of use killed the Pi Hut Fermion-brand board I had installed, making rotations and clicks unreliable.
...
There are three tables - `users`, `transactions`, and `transactionlog`. The third table records all writes to `transactions` as a checksum, but it serves no further purpose, and so I will focus on `users` and `transactions`.
...
this table associates RFID data with CRSIDs, and includes a checksum for the transaction history.
...
The python backend defines a RESTful API for running readonly queries on the database, allowing all embers of my group to check their balances in real time. I documented this using [flasgger](https://github.com/flasgger/flasgger), which after an hour or two of setup pain made the whole process relatively straightforward and clean.

# Conclusion
This was a very fun project, which seems to a) be fit for purpose, and b) genuinely be a popular way to track coffee consumption (in no small part thanks to the competitive streak brought out by the leaderboard).
